...
(defun reproduce-2 (parent-a parent-b)
  "Returns a single individual offspring from two individuals"
  (let ((crossover-index1 (1+ (random (1- (floor (/ (length parent-a) 2))))))
        (crossover-index2 (+ (floor (/ (length parent-a) 2)) (1+ (random (ceiling(1- (/ (length parent-a) 2)))))))
        (coin-flip (random 2)))
        (if (= coin-flip 0)
           (append
           ...
           (append
             (subseq parent-b 0 crossover-index1)
             (subseq parent-a crossover-index1 crossover-index2)
             (subseq parent-b crossover-index2 (length parent-b))))))
...
(defun inversion (individual)
  ...
  (let ((index1 (1+ (random (1- (floor (/ (length individual) 2))))))
        (index2 (+ (floor (/ (length individual) 2)) (1+ (random (ceiling(1- (/ (length individual) 2))))))))
      (append
             (subseq individual 0 index1)
             (reverse (subseq individual index1 index2))
             (subseq individual index2 (length individual)))))
    ...
    (
      ...
      (new-population
        (loop for _ in *population*
          ...
          collect (if (NOT (EQUAL (FIND possibly-inverted-child *population* :TEST #'EQUAL) NIL))
